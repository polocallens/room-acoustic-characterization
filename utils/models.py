...
#---------------------------------------------------------------------------------
def CRNN2D_5(X_shape, nb_classes):
    frequency_axis = 1
    time_axis = 2
    channel_axis = 3
    ...
    # Construct model
    model = Sequential()
    model.add(Conv2D(filters=256, kernel_size=(5, 5), input_shape=(X_shape[1], X_shape[2], X_shape[3]), activation='relu'))
    ...
    # recurrent layer
    model.add(GRU(64, activation='relu', recurrent_activation='relu', return_sequences=True))
    model.add(GRU(64, activation='relu', recurrent_activation='relu', return_sequences=False))
...
#---------------------------------------------------------------------------------
def CRNN2D_3(X_shape, nb_classes):
    frequency_axis = 1
    time_axis = 2
    channel_axis = 3
...
#---------------------------------------------------------------------------------
# ---------------------------------------------------------------------------------
def CRNN2D_4(X_shape, nb_classes):
    nb_layers = 4  # number of convolutional layers
    nb_filters = [64, 128, 128, 256]  # filter sizes
...
#---------------------------------------------------------------------------------
def CNN2(X_shape, nb_classes):
    # Looney, David, and Nikolay D. Gaubitch. "Joint Estimation Of Acoustic Parameters From Single-Microphone Speech Observations." ICASSP 2020-2020 IEEE International Conference on Acoustics, Speech and Signal Processing (ICASSP). IEEE, 2020.
    filter_size = 2
